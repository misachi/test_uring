/* Non-Vectored I/O with io_uring */
...
static struct io_uring_sqe *init_sqe(struct sq_ring *s_ring, uint64_t off, int fd, uint8_t op, char *buf, int len)
{
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        {
            init_sqe(s_ring, offset, fd, IORING_OP_READ, ptr, BLKSZ);
            offset += BLKSZ;
            ptr += BLKSZ;
        }
